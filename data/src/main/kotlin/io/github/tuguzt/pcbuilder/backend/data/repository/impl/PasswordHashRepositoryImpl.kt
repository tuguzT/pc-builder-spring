...
internal class PasswordHashRepositoryImpl(private val dataSource: PasswordHashDataSource) : PasswordHashRepository {
    override suspend fun hash(password: String): Result<String, Nothing?> =
        dataSource.hash(password)

    override suspend fun verify(hash: String, password: String): Result<Boolean, Nothing?> =
        dataSource.verify(hash, password)
}
